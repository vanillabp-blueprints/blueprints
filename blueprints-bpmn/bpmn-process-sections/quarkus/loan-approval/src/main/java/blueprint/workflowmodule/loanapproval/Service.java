...
public class Service {
  ...
  public void assessCreditRating(
    ...
    loanApproval.setLargeExposure(loanApproval.getAmount() >= properties.riskAssessmentFrom());
  ...
  public void collectDocuments(
    ...
    final var requiredDocuments = properties.documentsExpected() + (loanApproval.isRiskAssessmentRequired() ? 1 : 0);

    loanApproval.setDocumentCheck(
        DocumentCheck
            .builder()
            .documentsRequired(requiredDocuments)
            .documentsReceived(properties.documentsExpected())
            .build());

    log.info(
        "Loan approval '{}' received {} of {} documents",
        loanApproval.getLoanRequestId(),
        properties.documentsExpected(),
        requiredDocuments);
  ...
  public void checkCollateral(
    ...
    final var collateral = Math.min(
        (amount * properties.collateralPercentage()) / 100,
        properties.maximumCollateral());
    final var requiredCollateral = (amount * properties.minimumCollateralPercentage()) / 100;
  ...
  public void decideWithRiskReport(
    ...
    final var debtIsBearable = riskAssessment.getDebtRatio() <= properties.maximumDebtRatio();
  ...
  public void decideOnDocumentsAlone(
    ...
    final var ratingIsGoodEnough = loanApproval.getCreditRating() >= properties.minimumRating();
