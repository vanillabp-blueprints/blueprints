...
Starts the workflow through its BPMN message start event instead of an explicit call. A
delta on top of `module-single`, and a small one: one method name and one element in the
model.
...
`LoanRequested` is the message: the constant `Workflow.LOAN_REQUESTED` and the name of the
`bpmn:message` the start event references. If the two drift apart, the call publishes a
message no process subscribes to and no workflow starts - the call itself does not fail.
...
| `loan-approval/src/main/resources/loan-approval/processes/<adapter-id>/loan_approval.bpmn` | the process, whose start event carries a `bpmn:messageEventDefinition` referencing a `bpmn:message`                                           |
...
| `loan-approval/src/main/java/.../loanapproval/Workflow.java`                               | `ProcessService#startWorkflowByMessage` and the message name. The ONLY class using `ProcessService`                                           |
...
For a process modelled with a message start event, replace `startWorkflow` by
`startWorkflowByMessage(aggregate, messageName)` and keep everything else: the aggregate is
still created and persisted by the application, in the same transaction, and only its id
travels to the BPMS. Do not put message content into the call - there is no parameter for
it, on purpose. Whether a process starts by message is decided by the model, so read the
BPMN before choosing the method.
