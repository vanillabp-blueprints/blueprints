...
delta on top of [`module-multi`](https://github.com/vanillabp-blueprints/module-multi-quarkus),
...
repeated here: each module brings the index which makes its classes visible, carries the index which
makes its classes visible, keeps its resources below a directory of its own, and the
...
Wherever the engine shows its deployments, the two processes stand side by side and, between
them, nothing: no message flow, no call activity, no relation the engine knows about. Their
names carry the prefix of their module there, as `module-multi` explains.
